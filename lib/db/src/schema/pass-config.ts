...
export const passConfigTable = pgTable("pass_config", {
  ...
  currentPrice: numeric("current_price", { precision: 10, scale: 2 }).notNull().default("249"),
  originalPrice: numeric("original_price", { precision: 10, scale: 2 }).notNull().default("429"),
  pricingPeriodName: text("pricing_period_name").notNull().default("Super Early Bird"),
